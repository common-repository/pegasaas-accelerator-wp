<?php if (PegasaasAccelerator::$settings['status'] > 0 ) {
	...
	var desktop_accelerated_data = {
    datasets: [{
        data: [<?php echo $score_data['desktop_score']; ?><?php foreach ($score_data['opportunities'] as $rule) { ?>,<?php echo ($rule['impact']); } ?>],
 		backgroundColor:[<?php if ($score_data['desktop_score'] >= 90) { print "'#5cb85c'"; } else if ($score_data['desktop_score'] >= 50) {
...
<?php } ?>
	...
	var mobile_accelerated_data = {
    datasets: [{
        data: [<?php echo $score_data['mobile_score']; ?><?php foreach ($score_data['opportunities'] as $rule) { ?>,<?php echo ($rule['impact']); } ?>],
 		backgroundColor:[<?php if ($score_data['mobile_score'] >= 90) { print "'#5cb85c'"; } else if ($score_data['mobile_score'] >= 50) {
	print "'#f0ad4e'"; } else { print "'#cc0000'"; } ?><?php 
		foreach ($score_data['opportunities'] as $rule) { 
			print ","; 
			if ($rule['impact'] < 2 || true) { 
